...
**Choice:** Formalize a decision heuristic for "what mechanism should this feature use" — `IGameSystem`/tick, an immediate direct runtime call, pending-intent + tick consumption, a domain event, or scheduled work — chosen by the feature's actual behavior (does it need continuous evaluation, tick ordering, cross-thread sync inherent to the problem, multiple consumers) rather than by which abstraction happens to be available. Triggered by an architecture audit (`docs/audit/ZENITH_ARCHITECTURE_AUDIT.md`) that initially questioned whether `ChatSystem`/`GameModeSystem`/`EquipmentSystem` belong in `IGameSystem` — the risk flagged on review was replacing "everything is a System" with an equally rigid "everything discrete becomes a Command/Event", which would recreate the same friction under a different name.

1. **Strict invariants (never trade without a new ADR + concrete forcing feature):** transport doesn't decide gameplay; serialization doesn't know domain; packets are wire-only; handlers interpret and call a runtime API, they don't become the decision; authoritative world/player mutation normally has one writer in its gameplay execution context (the tick where ordering matters; exceptions must be explicit and justified); thread ownership is explicit; no async continuation mutates authoritative state outside a protected boundary.
...
4. Full decision heuristic, invariant/mechanism split, and the re-audit of `Chat`/`GameMode`/`Equipment`/`ChunkStream` against this lens: [`docs/adr/0097-runtime-execution-model.md`](adr/0097-runtime-execution-model.md) and the audit's "Addendum" section.

**Non-goals:** this ADR does not mandate introducing Command/Event/Scheduler types now — the audit found no feature today with ≥2 real consumers or a genuine future-scheduling need beyond what `GravityPendingStore` already covers. It also does not reverse any `IGameSystem` registration — `Chat`/`GameMode`/`Equipment` stay as Systems (see audit addendum for the per-feature reasoning), only `EquipmentSystem`'s registration order changes (ZAR-009).
...
**Status (ago 2026):** Decided — not yet backed by an automated architecture test (tracked as ZAR-004 in the audit, planned Phase 7 of the refactor plan).
