...
**Release tag `v0.0.2-alpha`:** closes H1 rows 1–5 + §55/§56/§59 on `main`/`develop`. Notes: [`release-notes-template.md`](release-notes-template.md). Gravity (§57) + playerdata (§60) + dual-storage (§61) + World seams (§62) + noise (§63) + overworld features (§64) on develop after the tag. **Next product code:** biomes/ore or §61 spike when needed.
