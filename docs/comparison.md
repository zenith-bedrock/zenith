...
**Later (not now):** when pulling a leaf, use Dragonfly as the “essentials” cross-check and PocketMine **issue tracker / changelogs** as a catalog of operator footguns (recipe unregister, overrides everywhere, command DX, …). Steal *symptoms and edge cases*, not PM’s plugin override model or public extension surface. Zenith's small typed command core is already permitted; it remains protocol-independent and non-plugin-facing. Workflow table: [`dx.md`](dx.md) “How to use refs when implementing a leaf”.
