...
24. **Yes-next after H1:** see [`roadmap.md`](roadmap.md) — default product leaf is small **survival honesty** (Q-throw / drop TTL or death drops) after human smoke on a **fresh** §72 world; §61 Mojang only when import is the goal; smoke-bot `first10` for regression proof.
```
