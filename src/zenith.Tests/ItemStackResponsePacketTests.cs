...
/// <summary>
/// Byte-level wire checks for the fields ADR §90 found missing: containers/item_stack_id are
/// genuine optionals (decorative presence bool + the option's own bool), not bare values.
/// </summary>
public class ItemStackResponsePacketTests
{
    [Fact]
    public void Error_writes_status_requestId_then_false_false_no_containers()
    {
        ...
        Assert.False(stream.ReadBool()); // containers_presence
        Assert.False(stream.ReadBool()); // containers option
        Assert.True(stream.IsEndOfFile);
    }

    [Fact]
    public void Ok_with_stack_id_writes_double_presence_then_zigzag_payload()
    {
        ...
        Assert.True(stream.ReadBool()); // containers_presence
        ...
        Assert.True(stream.ReadBool()); // item_stack_id_presence
    ...
    [Fact]
    public void Ok_without_stack_id_writes_false_false_no_payload()
    {
        ...
        stream.ReadBool(); // containers_presence
        ...
        Assert.False(stream.ReadBool()); // item_stack_id_presence
