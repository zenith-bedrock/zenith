...
{
    private readonly AsyncLogSink _sink;

    public Logger() : this(new AsyncLogSink()) { }

    internal Logger(AsyncLogSink sink) => _sink = sink;
    ...
    /// Multiple <see cref="Logger"/> instances (server/raknet) may share one asynchronous sink
    /// for a single run's file; null means console-only.
    /// </summary>
    public TextWriter? FileWriter
    {
        get => _sink.FileWriter;
        set => _sink.FileWriter = value;
    }
    ...
    public void Log(LogLevel level, string message)
    {
        ...
        var fileMessage = FileWriter is null
            ? null
            : $"[{DateTime.Now:yyyy-MM-dd HH:mm:ss.fff}] {level.ToString().ToUpperInvariant(),-7} {StripMarkup(message)}";
        _sink.Enqueue(formattedMessage, fileMessage);
    }
